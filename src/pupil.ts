import { grayscale, Pupil } from './worker_scripts/util';
 ...
 */
const getValue = (pixels: number[], x: number, y: number, width: number): number => pixels[y * width + x];
 ...
 * @returns {Matrix} - integral image
 */
const getSumTable = (pixels: number[], width: number, height: number): Matrix => {
  const integralImage: Matrix = new Array(width);
  let sumx = 0;
  let sumy = 0;
 ...
 * @return {Pupil} coordinate of the bottom right corner and width of the best fitted pupil
 */
const getSinglePupil = (pixels: number[], width: number, height: number): Pupil => {
  const summedAreaTable = getSumTable(pixels, width, height);
  let bestAvgScore = 999999; // want to minimize this score
  let bestPoint: [number, number] = [0, 0]; // bottom right corner of best fitted pupil
 ...
 * @param  {TwoEyes} eyesObj - left and right detected eye patches
 * @return {TwoEyes} eyesObj - updated eye patches with information about pupils' locations
 */
export const getPupils = (eyesObj: TwoEyes): TwoEyes => {
  if (!eyesObj) {
    return eyesObj;
  }
  if (!eyesObj.left.blink) {
    eyesObj.left.pupil = getSinglePupil(Array.from(grayscale(eyesObj.left.patch.data, eyesObj.left.width, eyesObj.left.height)), eyesObj.left.width, eyesObj.left.height);
    eyesObj.left.pupil[0][0] -= eyesObj.left.pupil[1];
    eyesObj.left.pupil[0][1] -= eyesObj.left.pupil[1];
  }
  if (!eyesObj.right.blink) {
    eyesObj.right.pupil = getSinglePupil(Array.from(grayscale(eyesObj.right.patch.data, eyesObj.right.width, eyesObj.right.height)), eyesObj.right.width, eyesObj.right.height);
